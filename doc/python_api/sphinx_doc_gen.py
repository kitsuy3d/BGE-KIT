...
INFO_DOCS = (
    ("info_quickstart.rst",
     "Range/Python Quickstart: new to Range/scripting and want to get your feet wet?"),
    ("info_overview.rst",
     "Range/Python API Overview: a more complete explanation of Python integration"),
    ("info_api_reference.rst",
     "Range/Python API Reference Usage: examples of how to use the API reference docs"),
    ("info_best_practice.rst",
     "Best Practice: Conventions to follow for writing good scripts"),
    ("info_tips_and_tricks.rst",
     "Tips and Tricks: Hints to help you while writing scripts for Range"),
    ("info_gotcha.rst",
...
REFERENCE_NAME = "range_python_reference_%s" % BLENDER_VERSION_PATH
...
def pycontext2sphinx(basepath):
    ...
    fw("\n")
    fw("The context members available depend on the area of Range which is currently being accessed.\n")
    fw("\n")
...
def pyrna2sphinx(basepath):
    ...
    if "bpy.types" not in EXCLUDE_MODULES:
        ...
        def fake_bpy_type(class_value, class_name, descr_str, use_subclasses=True):
            ...
            fw("   %s\n\n" % descr_str)
            fw("   .. note::\n\n")
            fw("      Note that bpy.types.%s is not actually available from within Range,\n"
...
def write_sphinx_conf_py(basepath):
    ...
    fw("project = 'Range Engine'\n")
    fw("master_doc = 'contents'\n")
    ...
    fw("latex_documents = [ ('contents', 'contents.tex', 'Range Index', 'Blender Foundation', 'manual'), ]\n")
...
def write_rst_contents(basepath):
    ...
    if not EXCLUDE_INFO_DOCS:
        fw(title_string("Range/Python Documentation", "=", double=True))
    ...
    if "bge" not in EXCLUDE_MODULES:
        fw(title_string("Range Game Engine Modules", "=", double=True))
        fw(".. toctree::\n")
        fw("   :maxdepth: 1\n\n")
        fw("   bge.types.rst\n\n")
        fw("   bge.logic.rst\n\n")
    ...
    fw("\n")
    fw("\n")
    fw(".. note:: The Range Python API has areas which are still in development.\n")
    #fw("   \n")
...
def write_rst_bpy(basepath):
    ...
    if ARGS.bpy:
        ...
        title = ":mod:`bpy` --- Range Python Module"
...
def write_rst_data(basepath):
    ...
    if "bpy.data" not in EXCLUDE_MODULES:
        ...
        fw(".. module:: bpy\n")
        fw("\n")
        fw("This module is used for all Range/Python access.\n")
        fw("\n")
        fw(".. data:: data\n")
        fw("\n")
        fw("   Access to Range's internal data\n")
        fw("\n")
