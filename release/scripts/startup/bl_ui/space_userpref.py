...
class USERPREF_MT_splash(Menu):
    ...
    def draw(self, context):
        ...
        if not text:
            #context.user_preferences.view.use_pro_mode = False
...
def scriptrunner():
    ...
    for i, q in allUnframed:
        allUnframed[i].outline = (0.500, 0.200, 0.000)
        allUnframed[i].apply(gui, q)
...
class USERPREF_MT_splash_footer(Menu):
    ...
    def draw(self, context):
        ...
        userpref = context.user_preferences
        view = userpref.view
        if view.use_startup_mode==True:
            view.use_startup_mode = False
            view.use_pro_mode = False
            userpref.system.use_region_overlap = True
            userpref.system.anisotropic_filter = 'FILTER_0'
            userpref.system.image_draw_method = 'GLSL'
            userpref.filepaths.save_version = 0
            userpref.filepaths.recent_files = 20
        ...
        if view.use_pro_mode==True:
            ...
            col = row.column()
            col.label("Pro Mode")
            col.prop(view, "use_pro_mode", text="Pro Mode Is On", icon='GAME')
        else:
            row.label("")
            col = row.column()
            col.label("You Can Do It!")
            col.prop(view, "use_pro_mode", text="Pro Mode Is Off", icon='BLENDER')
