...
def updateScripts(dummy):
    ...
    if updateScripts in bpy.app.handlers.scene_update_post: # workaround to execute updateScripts after the addon is registered
        bpy.app.handlers.scene_update_post.remove(updateScripts)
...
class PropertyMsgList(UIList):
    
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        row = layout.row(align=True)
        row.prop_search(item, "prop", context.object.game, "properties", text="")
        row.prop(item, "type", text="")
        row.operator("eo.remove_msg", text="", icon="X").index = index
         
class TransmissionList(UIList):
    
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        row = layout.row(align=True)
        
        row.label(icon="LAMP_AREA")
        row.prop(item, "name", text="", emboss=False)
        #row.operator("eo.remove_trans", text="", icon="X").index = index
        #row.prop(item, "type",text="")
...
class EO_Object_Settings(PropertyGroup):
    ...
    types = [
    ("TRANSMITTER", "Transmitter", "Transmitter update their attributes - for example position - at other clients/server", "LAMP_AREA", 1),
    ("AVATAR", "Avatar", "Object used to represent a Transmitter. The avatar will adopt the data received from the transmitter. Must be in inactive layer.", "MOD_MIRROR", 2),
...
class EOPanel(bpy.types.Panel):
    ...
    def draw(self, context): 
        ...
        box = layout.box()
        box.label("Setting Templates", icon = "SCRIPTPLUGINS")
        ...
        row.operator("eo.open_template",   text="", icon="FILESEL")
        row.operator("eo.remove_net",      text="", icon="X")
        col1.operator("eo.apply_template")
        ...
        if obj.eo.type != "NONE":
            ...
            if obj.eo.type == "TRANSMITTER":
                box.label("Transmission Settings", icon="WORLD")
                rep = box.row()
                rep.prop(obj.eo, "avatar", icon="MOD_MIRROR")
                rep.enabled = not obj.eo.global_object
                box.prop(obj.eo, "global_object")
                
                box.separator()
                box.operator("eo.add_trans", icon="ZOOMIN")
                row = box.row()
                col1 = row.column()
                col2 = row.column()
                col1.template_list("TransmissionList", "", obj.eo, "trans_attrs", obj.eo, "trans_attr_index")
                ...
                if obj.eo.trans_attrs:
                    if obj.eo.trans_attr_index != -1:
                        ...
                        if item.type == "GAME_PROPERTY":
                            ...
                            col2.prop(item, "Custom", text="kx property")
                        col2.operator("eo.remove_trans", text="Delete", icon="X").index = obj.eo.trans_attr_index
            elif obj.eo.type == "AVATAR":
                box.label("Avatar Settings", icon="WORLD")
                box.operator("eo.add_msg", icon="ZOOMIN")
                box.template_list("PropertyMsgList", "", obj.eo, "avat_msg", obj.eo, "avat_msg_index")

            box.operator("eo.save_template", icon="FILE_TICK")                    

        """
        layout.separator()
        
        box = layout.box()
        box.label("Communication Settings", icon="WORLD")
        box.prop(obj.eo, "message_object")
        box.label("Direct Message Properties:")
        box.template_list("FeedbackList", "", obj.game, "properties", obj.eo, "feedback_index")
        """
...
class EOScenePanel(bpy.types.Panel):
    """Creates a Panel in the Logic properties window"""
    bl_label = "Easy Online"
    bl_idname = "OBJECT_PT_EO_SCENE"
...
def register():
    ...
    for handlers in [bpy.app.handlers.load_post, bpy.app.handlers.save_pre, bpy.app.handlers.game_pre, bpy.app.handlers.scene_update_post]:
